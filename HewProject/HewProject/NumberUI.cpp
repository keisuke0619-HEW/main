...
CNumberUI::CNumberUI(int length, SortOrder sort, bool isFloat, int floatLength)
{
	m_rot = 0;
	...
	ResetLength(length, sort, isFloat, floatLength);
...
void CNumberUI::ResetLength(int length, SortOrder sort, bool isFloat, int floatLength)
{
	for (int i = 0; i < m_ui.size(); i++)
	{
		CUIManager::GetIns()->Delete(m_ui[i]);
...
void CNumberUI::RePosition()
{
	...
	for (int i = 0; i < m_ui.size(); i++)
	{
		...
		if (m_isFloat && i == m_len - m_floatLen - 1)
		{
			auto pointpos = pos;
